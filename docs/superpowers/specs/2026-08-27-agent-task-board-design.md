...
**ЖЁСТКОЕ ПРАВИЛО — никаких локальных файлов, только MCP.** Первый раздел тела скилла (сразу
после триггера, до жизненного цикла) — блокирующее правило в духе `FORBIDDEN`-секции из
основного `memory-mcp/SKILL.md`, продублированное явно здесь, а не через отсылку "см. другой
скилл" (модель не обязана держать оба скилла в контексте одновременно):

> **FORBIDDEN**: план, аналитика, прогресс по подзадачам, результаты тестов/ревью и финальный
> отчёт — всё это идёт **только** через MCP-инструменты (`agent_task_create`, `agent_task_update`,
> `memory_save`), никогда через `Write`/`Edit` в локальный файл — ни в проекте, ни во временную
> директорию, ни в `docs/`, ни куда-либо ещё. Это не "предпочтительно", это безусловный запрет,
> без исключений, специфичных для этого скилла. В частности:
> - Текст плана из ANALYSIS-подзадачи → `agent_task_update(..., description: ...)`, не файл.
> - Summary каждой IMPLEMENTATION/TESTING/REVIEW-подзадачи → `description` через
>   `agent_task_update`, не `*.md`/`*.txt` рядом с кодом.
> - Финальный отчёт (раздел 6) → `memory_save(type: "REPORT", ...)`, не `.html`-файл на диске.
> - Единственное исключение — то же, что и в основном `SKILL.md`: реальный исходный файл,
>   который пользователь явно попросил создать/изменить как часть кода приложения. Всё
>   остальное — в память через MCP.
